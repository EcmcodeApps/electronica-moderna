...
function actualizarBanner(producto) {
  ...
  if (esPromo) {
    var txt = producto.texto_promo || 'PROMOCIÓN';
    if (elBannerTexto) elBannerTexto.textContent =
      '⚡ ' + txt + ' ⚡ ' + txt + ' ⚡ ' + txt + ' ⚡';
    elBanner.classList.add('visible');
    if (elBadgePromo) { elBadgePromo.textContent = '🏷️ ' + txt; elBadgePromo.style.display = 'block'; }
  } else {
    elBanner.classList.remove('visible');
    if (elBadgePromo) elBadgePromo.style.display = 'none';
  }
